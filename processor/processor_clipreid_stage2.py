...
def do_train_stage2(cfg,
    ...
    labels = []
    with torch.no_grad():
        # Changed unpacking to match your dataset: (clip, label, dog_id, video_id)
        for n_iter, (img, vid, dog_id, video_id) in enumerate(train_loader_stage1):
            img = img.to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        model.train()
        # Changed unpacking to match your dataset
        for n_iter, (img, vid, dog_id, video_id) in enumerate(train_loader_stage2):
            optimizer.zero_grad()
            optimizer_center.zero_grad()
            img = img.to(device)
            target = vid.to(device)
            
            # Force SIE embeddings off, ignore dog_id and video_id strings
            target_cam = None
            target_view = None
            with amp.autocast(enabled=True):
        ...
        # --- PASTE THE STANDALONE MODEL SAVER HERE ---
        checkpoint_period = getattr(cfg.SOLVER.STAGE2, "CHECKPOINT_PERIOD", 20)
        if epoch % checkpoint_period == 0 or epoch == epochs:
            save_name = os.path.join(cfg.OUTPUT_DIR, f'dog_model_ep{epoch}.pth.tar')
            save_checkpoint(model.state_dict(), False, save_name)
            logger.info(f"Saved model checkpoint to {save_name}")

        # Added safety check for val_loader so it doesn't crash when None
